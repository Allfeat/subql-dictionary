...
import frontierEvmProcessor from '@subql/frontier-evm-processor';
...
export async function handleBlock(block: SubstrateBlock): Promise<void> {
  ...
  for (const evt of wrappedEvents.filter(evt => {
    const baseFilter = frontierEvmProcessor.handlerProcessors['substrate/FrontierEvmEvent'].baseFilter[0];
  ...
  for (const ex of wrappedExtrinsics.filter(ex => {
    const baseFilter = frontierEvmProcessor.handlerProcessors['substrate/FrontierEvmCall'].baseFilter[0];
...
async function handleEvmLog(blockNumber: string, event: SubstrateEvent): Promise<EvmLog[]> {
  ...
  const evmLogs = await frontierEvmProcessor.handlerProcessors['substrate/FrontierEvmEvent'].transformer({
    input: event,
    ds: {} as any,
    api: api as any,
  });
...
async function handleEvmTransaction(idx: number, tx: SubstrateExtrinsic): Promise<EvmTransaction[]> {
  const calls = await frontierEvmProcessor.handlerProcessors['substrate/FrontierEvmCall'].transformer({
    input: tx,
    ds: {} as any,
    api: api as any,
  });
